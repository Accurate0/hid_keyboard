...
public:
    ...
    public:
        ...
        static hid_device* construct_handle(uint16_t vid, uint16_t pid, uint16_t usage, uint16_t usage_page) {
...
int main(void)
{
    ...
    [](struct libusb_context *ctx, struct libusb_device *dev, libusb_hotplug_event event, void *user_data) -> int
    {
        ...
        if (event == LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED) {
            ...
            auto new_handle_or_null = HID::Device::construct_handle(VENDOR_ID, PRODUCT_ID, USAGE, USAGE_PAGE);
    ...
    (void)libusb_hotplug_register_callback(nullptr,
    ...
    std::cerr << "hotplug support = " << (libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG) ? "true" : "false") << "\n";
